...
> Smoke-run timings, not steady-state: the total includes one-time MPS kernel compilation / warmup (no separate warmup pass is run). This run is **partial** (aborted in the diffusion sampler when the SVD CPU fallback failed to converge on degenerate synthetic coords), so `diffusion_sampler` is incomplete and the confidence head did not run.
...
| trunk_msa_module | 1780 | 29 | - (none observed) |
| trunk_pairformer | 12672 | 25 | - (none observed) |
